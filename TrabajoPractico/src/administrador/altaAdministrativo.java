...
public class altaAdministrativo extends JFrame {

	private JPanel contentPane;
	private JTextField txtNombre;
	private static altaAdministrativo altaAdministrativo;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					altaAdministrativo = new altaAdministrativo();
					altaAdministrativo.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public altaAdministrativo() {
		...
		btnAltaTecnico.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				administrativo = Controlador.crearAdministrativo(txtNombre.getText().toString(),
				...
				if (administrativo.getDni() != -1 && administrativo.getUsuario() != "-1") {
					p = new popUp("ADMINISTRATIVO CON DNI: " + administrativo.getDni() + " Y USUARIO: "
							+ administrativo.getUsuario() + " CREADO CON EXITO");
					txtNombre.setText("");
					txtApellido.setText("");
					txtDNI.setText("");
				...
				} else if (administrativo.getDni() == -1 && administrativo.getUsuario() == "-1")
					p = new popUp("YA EXISTE UN ADMINISTRATIVO CON EL DNI: " + txtDNI.getText().toString()
							+ " Y USUARIO: " + txtUsuario.getText().toString());
				else if (administrativo.getDni() == -1)
					p = new popUp("YA EXISTE UN ADMINISTRATIVO CON EL DNI: " + txtDNI.getText().toString());
				else
					p = new popUp("YA EXISTE EL USUARIO: " + txtUsuario.getText().toString());

				p.setLocationRelativeTo(altaAdministrativo);
				p.setVisible(true);

				Controlador.listarAdministrativos();
