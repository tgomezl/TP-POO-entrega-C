...
public class altaTecnico extends JFrame {

	private JPanel contentPane;
	private JTextField txtNombre;
	private DefaultComboBoxModel<String> listaTipoTecnico = new DefaultComboBoxModel<>();
	private DefaultComboBoxModel<String> listaTipoTurno = new DefaultComboBoxModel<>();
	private static altaTecnico altaTecnico;
	...
	public altaTecnico() {
		...
		comboTipoTecnico.setBounds(112, 148, 86, 22);
		panelAltaTecnico.add(comboTipoTecnico);

		comboTipoTecnico.setModel(listaTipoTecnico);

		for (TipoTecnico t : Controlador.obtenerTiposTecnicos()) {
			listaTipoTecnico.addElement(t.getTipo().toString());
		}

		JLabel lblNewLabel_1 = new JLabel("TIPO TECNICO:");
		lblNewLabel_1.setHorizontalAlignment(SwingConstants.RIGHT);
		lblNewLabel_1.setFont(new Font("Tahoma", Font.ITALIC, 10));
		lblNewLabel_1.setBounds(10, 152, 92, 18);
		panelAltaTecnico.add(lblNewLabel_1);
		...
		JComboBox comboTipoTurno = new JComboBox();
		comboTipoTurno.setBounds(271, 148, 86, 22);
		panelAltaTecnico.add(comboTipoTurno);

		comboTipoTurno.setModel(listaTipoTurno);

		for (TipoTurno t : TipoTurno.values()) {
			listaTipoTurno.addElement(t.toString());
		}
		...
		btnAltaTecnico.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				TipoTecnico tipo = null;

				for (TipoTecnico t : Controlador.obtenerTiposTecnicos()) {
					if (t.getTipo().toString().equals(comboTipoTecnico.getSelectedItem().toString())) {
						tipo = new TipoTecnico();
						tipo.setTipo(t.getTipo());
					}
				}
				...
				for (TipoTurno t : TipoTurno.values()) {
					if (t.toString().equals(comboTipoTurno.getSelectedItem().toString()))
						tipoTurno = t;
				}
				...
				tecnico = Controlador.crearTecnico(txtNombre.getText().toString(), txtApellido.getText().toString(),
						Integer.parseInt(txtDNI.getText().toString()), sexo,
						...
						txtContrasena.getText().toString(), tipo, tipoTurno);

				popUp p;

				if (tecnico.getDni() != -1 && tecnico.getUsuario() != "-1") {
					p = new popUp("TECNICO CON DNI: " + tecnico.getDni() + " Y USUARIO: " + tecnico.getUsuario()
							+ " CREADO CON EXITO");
					txtNombre.setText("");
					txtApellido.setText("");
					txtDNI.setText("");
				...
				} else if (tecnico.getDni() == -1 && tecnico.getUsuario() == "-1")
					p = new popUp("YA EXISTE UN TECNICO CON EL DNI: " + txtDNI.getText().toString() + " Y USUARIO: "
							+ txtUsuario.getText().toString());
				else if (tecnico.getDni() == -1)
					p = new popUp("YA EXISTE UN TECNICO CON EL DNI: " + txtDNI.getText().toString());
				else
					p = new popUp("YA EXISTE EL USUARIO: " + txtUsuario.getText().toString());

				p.setLocationRelativeTo(altaTecnico);
				p.setVisible(true);
		...
		btnAltaTecnico.setBounds(148, 217, 142, 23);
		...
		lblNewLabel_1_1_5_1.setBounds(10, 181, 92, 18);
		panelAltaTecnico.add(lblNewLabel_1_1_5_1);

		txtUsuario = new JTextField();
		txtUsuario.setColumns(10);
		txtUsuario.setBounds(112, 180, 86, 20);
		...
		lblNewLabel_1_1_2_2_1.setBounds(169, 182, 92, 18);
		panelAltaTecnico.add(lblNewLabel_1_1_2_2_1);

		txtContrasena = new JPasswordField();
		txtContrasena.setBounds(271, 180, 86, 18);
		...
		lblNewLabel_1_2.setBounds(169, 152, 92, 18);
		panelAltaTecnico.add(lblNewLabel_1_2);
